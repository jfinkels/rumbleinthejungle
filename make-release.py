...
def set_file_version(version):
    info('Setting rumbleinthejungle.py version to %s', version)
    set_filename_version('rumbleinthejungle.py', version, '__version__')
...
def fail(message, *args):
    logging.error('Error: %s', message % args)
    sys.exit(1)


def info(message, *args):
    logging.info('%s', message % args)
